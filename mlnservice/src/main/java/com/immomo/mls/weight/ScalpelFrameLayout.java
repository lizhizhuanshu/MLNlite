...
public class ScalpelFrameLayout extends FrameLayout {
    ...
    private boolean isDrawViewNames = false;
    ...
    public boolean isDrawViewNames() {
        return isDrawViewNames;
    }

    public void setDrawViewNames(boolean drawViewNames) {
        isDrawViewNames = drawViewNames;
    }
    ...
    public void draw(@SuppressWarnings("NullableProblems") Canvas canvas) {
        if (!enabled) {
            ...
            super.draw(canvas);
            long cast = System.currentTimeMillis() - now;
            if (cast > 16) {
                LogUtil.e("draw lua view cast: " + cast);
            }
            return;
        }
        ...
        while (!layeredViewQueue.isEmpty()) {
            ...
            } else if (isDrawViewNames) {
                String className = view.getClass().getSimpleName() + " " + view.getMeasuredWidth() + "X" + view.getMeasuredHeight();
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        long cast = System.currentTimeMillis() - now;
        if (cast > 16) {
            LogUtil.e("layout lua view cast: " + cast);
        }
    }
}