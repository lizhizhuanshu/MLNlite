...
static void _free_double(void*v){
    double *d = (double *) v;
    m_malloc(d, sizeof(double), 0);
}

static void* _mallocDouble(double d){
    double *p = m_malloc(NULL, 0, sizeof(double));
    *p = d;
    return p;
}
...
static Map *initMethodsMap() {
    ...
    map_set_free(map, _free_str,_free_double);
    return map;
}
...
void statistics_searcher_Call(const char *fromType, const char *filename, double time) {
    ...
    map_put(files, copystr(filename), _mallocDouble(time));
#endif
}
